...
def brezenham_ellipse(a, b, x_center, y_center):
    ...
    while y > 0:
        ...
        if delta <= 0:  # Если внутри эллипса или на нем
            ...
            delta_1 = 2 * delta + a2 * (2 * y - 1)  # Разность расстояний горизонтального и диагонального пискелей до окружности
            if delta_1 < 0:  # Горизонтальный ближе
                x += 1
                delta += b2 * (2 * x + 1)
            else:  # Диагональный ближе
            ...
            delta_2 = 2 * delta - b2 * (2 * x + 1) # Разность расстояний диагонального и вертикального пискелей до окружности
            if delta_2 < 0:  # Диагональный ближе
                x += 1
                y -= 1
                delta += a2 * (1 - 2 * y)
